...
function EditBimbingan() {
  ...
  useEffect(() => {
    const fetchBimbinganDetail = async () => {
      const token = localStorage.getItem("access");

      try {
        const response = await fetch(`http://localhost:8000/api/detail-bimbingan/${id}/`, {
          headers: {
        ...
        if (response.ok) {
          setFormData({
            tahunSemester: data.tahun_semester,
            namaKegiatan: data.nama,
            deskripsi: data.deskripsi_kegiatan,
            tanggalMulai: data.tanggal_mulai.slice(0, 16),
            tanggalSelesai: data.tanggal_selesai.slice(0, 16),
            durasi: data.durasi || 0,
            tipePenyelenggaraan: data.tipe_penyelenggaraan,
          ...
          alert("Gagal mengambil data bimbingan");
        }
      } catch (error) {
    ...
    const fetchPembimbing = async () => {
      const token = localStorage.getItem("access");
      try {
        const response = await fetch("http://localhost:8000/api/list-pembimbing/", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    form.append("nama_dokumen", formData.namaDokumen);
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/edit-bimbingan/${id}/`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      alert("Terjadi kesalahan saat memperbarui: " + err.message);
    }
    }
